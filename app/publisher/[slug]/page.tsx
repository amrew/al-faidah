...
export default async function Articles({
  ...
  const [{ data: contents }, { count }] = await Promise.all([
    supabase
      ...
      .select<any, ArticleSummaryType>(
        "id, title, slug, summary, image, created_at, read_stats, taxonomies( slug, name ), publishers!inner( title, slug, logo_url, web_url )"
      )
      .eq("publishers.slug", params.slug)
  ...
  const createUrl = (page: number) => {
    return `/publisher/${params.slug}?page=${page}`;
  };
